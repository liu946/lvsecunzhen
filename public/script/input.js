...
var getDBvalue, getformvalue, getinputname, getkeyvalue, inputs, putmodel, value, zhenqustorge;
...
putmodel = function(object, inputs) {
  var b, data, datatype, fieldid, fieldname, html, i, id, items, j, k, l, len, len1, len2, mend, o, onlyincity, p, ref, special, str, type, unit, v;
  html = "";
  id = object["class"];
  ...
  for (j = 0, len = ref.length; j < len; j++) {
    ...
    html += "<div class='list " + special + "' " + mend + "> <div class='note'> <h3>" + fieldname + "</h3> </div> <div class='input'> " + str + " </div> </div>";
  }
  html += "</div>";
  $('#J_form').append(html);
  return inputs;
...
$('.submit').on('click', function() {
  ...
  if (target.flag) {
    ...
    return $.post("/input/update/" + modelname, value, function(data) {
      alert('提交成功');
      return location.href = "/input/index/" + modelname;
    });
  }
});
